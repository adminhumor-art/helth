...
## Документация полного локального прототипа — 2 августа 2026

После предыдущего контрольного среза программный вертикальный цикл был связан:

- GS1/GS1Sb международного и китайского профилей проходят один пользовательский
  поток от кода и BLE-поиска до изолированной диагностики;
- свежая диагностическая точка может быть активирована только явной кнопкой
  «Использовать этот датчик»;
- exact manual activation в одной Room-транзакции повторно проверяет выбранный
  event, raw sample, результат ядра, protocol binding, checkpoint и SHA-256
  установленных нативных наборов, затем создаёт стабильную локальную binding;
- product foreground service читает только эту локальную binding, восстанавливает
  историю и проводит новые batch через долговечный local-alarm settlement до UI;
- крупное значение, график, product alarm, signal-loss watchdog и виджет получают
  только технически допущенное локальное показание;
- семейная credential и remote route отделены от локальной identity датчика;
  сайт получает только копию и не может менять датчик, историю, пороги,
  Bluetooth, виджет или локальную тревогу;
- экран семейного доступа подключён через настройки; live backend, защищённая
  browser session и настоящий Telegram всё ещё требуют отдельного deploy;
- GS3 не подменяется GS1 и пока не имеет полного Android onboarding,
  BLE/product driver и подтверждённого нативного пути.

Перед изменением документов проведён read-only аудит инструкций и текущего кода.
Устаревшие заявления о незавершённом wiring, отсутствии пользовательской
активации и неподключённом семейном экране заменены текущей фактической схемой.
Добавлены internet matrix, copy-only граница сайта, пошаговая ручная активация и
контрольный список первого Samsung/датчика.

Числа и итог финальных тестов здесь намеренно не записаны заранее. Следующая
запись должна содержать результат полного Android/backend/web preflight и SHA-256
собранного APK. Даже зелёный программный preflight не заменит физическую проверку
BLE/JNI, китайского профиля, калибровки, звука, DND, Doze и reboot на первом
устройстве.

## Устанавливаемый Android-срез — 2 августа 2026

Перед выдачей APK независимый обзор обнаружил P1: runtime identity комплекта
data-handle была ручной константой и не была связана с проверяемым manifest
фактически упакованных `.so`. Исправление выполнено по TDD: новый тест сначала
не компилировался без generated manifest, затем Gradle-сборка стала строить
Global/CN identity только из `native-binaries.sha256` после сверки всех четырёх
реальных библиотек. Изменение любого pinned hash меняет runtime identity и
инвалидирует прежний physical approval.

Итоговый Android-прогон включал все debug JVM-тесты, сборку APK приложения и
компиляцию Android-test APK для app, Room, transport, data-handle и алгоритма:
374 Gradle-задачи, `BUILD SUCCESSFUL`. После последнего P1-исправления отдельно
повторены полные unit-наборы data-handle и sensor и `app:assembleDebug` — успешно.

Файл `artifacts/Sladkaya-0.1.0-dev-debug.apk`:

- размер 32 793 686 байт;
- пакет `com.sladkaya.app.debug`, версия `0.1.0-dev-debug`, minSdk 26;
- APK Signature Scheme v2 — успешно;
- 16 KiB zip alignment — успешно;
- arm64-v8a и armeabi-v7a; отдельные Global/CN data-handle `.so` присутствуют;
- SHA-256:
  `860de66e22c78bc957cf157fcbb3c5943779977c01ce9f18a203084a40432f91`.

Этот срез готов для первого инженерного теста на Samsung. Он ещё не является
физически подтверждённым мониторингом: без телефона и датчика не проверены
реальный китайский BLE-сеанс, JNI на ARM, калибровочный путь, точность,
пробуждение звуком, DND/Doze и восстановление после перезагрузки.
