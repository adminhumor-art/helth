...
internal class Gs1PacketProcessor(
    ...
    suspend fun ingest(encryptedPacket: ByteArray): Gs1PacketProcessingResult = mutex.withLock {
        if (pending != null) {
            return@withLock Gs1PacketProcessingResult.PersistenceUnavailable(
                "The previous GS1 batch still requires a durable commit",
            )
        }
        val verified = when (val decoded = decoder.decode(encryptedPacket.copyOf())) {
    ...
    private fun validateWholeBatch(samples: List<DecodedGs1RawSample>): String? {
        ...
        samples.forEachIndexed { position, sample ->
            ...
            if (position > 0) {
                ...
                if (sample.sensorTimeEpochSeconds != previous.sensorTimeEpochSeconds + SECONDS_PER_SAMPLE) {
                    return "Sensor timestamps in one batch must advance by exactly 60 seconds"
                }
            }
        }
        return null
